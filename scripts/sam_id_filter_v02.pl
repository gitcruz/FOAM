...
#use Pod::Usage; # commented this to work at newcluster
# Accepts two parameters via commandline previously hard-coded: minmatch and minqual 

use Getopt::Long;
use File::Basename;
 
$this_program = basename($0);
#my $minmatch = 800; # old default
#my $minqual = 12;  # old default
my $minmatch = 5000; #new default
my $minqual = 13; #new default
my $target = 0;
my $result = GetOptions(
			'b|buffer=i' => \$printlines,
			'q|min_q=i'   => \$minqual,
                        'm|min_match=i'   => \$minmatch, #option cannot have same name as variable
			'u|uniq=s'   => \$uniq,
			'ref:s' => \$target
		       );

#print "using min_match $minmatch and min_q $minqual\n";
